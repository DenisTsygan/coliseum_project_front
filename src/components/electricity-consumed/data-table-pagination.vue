...
<template>
    <div class="flex items-center justify-between px-2 pt-4">
        <div class="flex-1 text-sm text-muted-foreground">
            ...
            {{ t("electricityConsumed.data-table-pagination.rows") }}
            .
        </div>
        <div class="flex items-center space-x-6 lg:space-x-8">
            <div class="flex items-center space-x-2">
                <p class="text-sm font-medium">
                    {{ t("electricityConsumed.data-table-pagination.rowsPerPage") }}
                </p>
                <Select :model-value="`${table.getState().pagination.pageSize}`"
                    @update:model-value="(n: string) => table.setPageSize(+n)">
            ...
            <div class="flex w-[100px] items-center justify-center text-sm font-medium">
                {{ t("electricityConsumed.data-table-pagination.page") }}
                {{ table.getState().pagination.pageIndex + 1 }}
                {{ t("electricityConsumed.data-table-pagination.of") }}
                {{ table.getPageCount() }}
            </div>
            <div class="flex items-center space-x-2">
